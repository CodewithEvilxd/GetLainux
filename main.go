...
var phrases = map[string]Phrases{
	"EN": {
		...
		Arch:        "Arch",
		Kernel:      "Kernel",
		ExitConfirm: "Exit GetLainux installer?",
		ExitAction:  "EXIT",
	...
	"RU": {
		...
		Arch:        "Архитектура",
		Kernel:      "Ядро",
		ExitConfirm: "Выйти из установщика GetLainux?",
		ExitAction:  "ВЫХОД",
...
func (m model) View() string {
	...
	case stepConfig:
		os.WriteFile("getlainux-config.txt", []byte("# Configuration saved\nType=minimal"), 0644)
		return docStyle.Render(fmt.Sprintf("%s\n\n%s\n\n%s",
