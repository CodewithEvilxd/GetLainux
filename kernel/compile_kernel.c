// this file for compily getlainux kernel from mcarchiso
...
bool create_missing_files() {
    ...
    if(access("airootfs/root/.automated_script/customize_airootfs.sh", F_OK) != 0) {
        ...
        if(fp) {
            ...
            fprintf(fp, "echo 'getlainux' > /etc/hostname\n\n");
...
bool validate_profiledef() {
    ...
    if(result != 0) {
        ...
        if(fp) {
            ...
            fprintf(fp, "iso_name=\"getlainux\"\n");
            fprintf(fp, "iso_label=\"GETLAINUX\"\n");
            fprintf(fp, "iso_publisher=\"GetLainux\"\n");
            fprintf(fp, "iso_application=\"GetLainux Live\"\n");
...
bool run_test_build() {
    ...
    char temp_dir[] = "/tmp/test-getlainux-XXXXXX";
    if(!mkdtemp(temp_dir)) {
...
bool load_config(const char *filename) {
    ...
    if (!fp) {
        ...
        strncpy(g_config.author, "Nishant Gaurav", sizeof(g_config.author) - 1);
