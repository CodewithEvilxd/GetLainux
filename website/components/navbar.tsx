...
export function Navbar() {
  ...
  const navItems = isGetLainuxPage ? getlainuxNavItems : isNexusPage ? nexusNavItems : []
        ...
        <div className="flex h-14 sm:h-16 items-center justify-between">
          <Link href="/" className="flex items-center space-x-1.5 sm:space-x-2 group">
            <div className="h-8 w-8 sm:h-10 sm:w-10 relative bg-transparent">
              <Image
                src={isNexusPage ? (logoError ? "/logo.png" : "/nexus-logo.png") : "/logo.png"}
                alt={isNexusPage ? "Nexus Logo" : "GetLainux Logo"}
                width={40}
                height={40}
                className="h-8 w-8 sm:h-10 sm:w-10 group-hover:scale-110 transition-transform bg-transparent"
                style={{ background: 'transparent', backgroundColor: 'transparent' }}
                priority
                onError={() => isNexusPage && setLogoError(true)}
              />
            </div>
            <span className="text-xl sm:text-2xl font-black font-heading">
              {isHomePage ? 'GetLainux Ecosystem' : isNexusPage ? 'Nexus' : 'GetLainux'}
            </span>
          </Link>
