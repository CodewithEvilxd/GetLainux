...
/**
 * Displays the welcome screen with GetLainux branding
...
void show_welcome_screen() {
    ...
    // Display GetLainux logo and title
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(y/2 - 2, (x-13)/2, "GETLAINUX");
...
int show_turbo_install_menu(DiskInfo disks[], int count) {
    ...
    while(1) {
        ...
        // Turbo Install title
        attron(A_BOLD | COLOR_PAIR(1));
        mvprintw((y-18)/2 - 1, (x-60)/2 + 2, " [ Turbo Install GetLainux ] ");
        attroff(A_BOLD | COLOR_PAIR(1));
        ...
        mvprintw((y-18)/2 + 6, (x-60)/2 + 6, "• User creation (getlainux:getlainux)");
