...
int auto_configure_system(void) {
    ...
    system("echo 'getlainux-pc' > /mnt/etc/hostname 2>/dev/null");
    ...
    system("echo '127.0.1.1 getlainux-pc.localdomain getlainux-pc' >> /mnt/etc/hosts 2>/dev/null");
    ...
    system("arch-chroot /mnt useradd -m -G wheel,audio,video,storage -s /bin/bash getlainux 2>/dev/null || true");
    system("echo 'getlainux:getlainux' | arch-chroot /mnt chpasswd 2>/dev/null");
...
int turbo_install(DiskInfo *disk) {
    printf("\n");
    printf("TURBO INSTALL GETLAINUX\n");
    printf("********************\n");
    ...
    FILE *fp = fopen("/mnt/etc/sysctl.d/99-getlainux.conf", "w");
    if (fp) {
    ...
    // Step 8.5: Custom Kernel Optimization
    printf("STEP 8.5: Custom Kernel Optimization\n");
    printf("  Applying kernel optimizations...\n");
    ...
    if (access("../../kernel/optimize_kernel.sh", F_OK) == 0) {
        ...
        printf("  Applying kernel optimizations (fallback method)...\n");
        
        // Write kernel optimization sysctl
        ...
        FILE *gfp = fopen("/mnt/etc/default/grub", "r+");
        if (gfp) {
            // Simple append if file exists
            fseek(gfp, 0, SEEK_END);
            fprintf(gfp, "\n# GetLainux Kernel Boot Optimizations\n");
            fprintf(gfp, "GRUB_CMDLINE_LINUX_DEFAULT=\"$GRUB_CMDLINE_LINUX_DEFAULT intel_idle.max_cstate=1 processor.max_cstate=1\"\n");
            fclose(gfp);
        }
    }
    
    printf("  Kernel optimization completed\n");
    ...
    printf("\n");
    printf("Credentials:\n");
    printf("  Username: getlainux\n");
    printf("  Password: getlainux\n");
    printf("\n");
    printf("Next steps:\n");
    printf("  1. Reboot your computer\n");
    printf("  2. Remove installation media\n");
    printf("  3. Boot into GetLainux\n");
