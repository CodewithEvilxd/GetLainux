...
void handle_sigint(int sig) {
    ...
    printf("\n[!] GetLainux Installer aborted by user. System clean.\n");
    exit(0);
}

/**
 * Main installation function for GetLainux
...
int install_lainux(DiskInfo *disk) {
    ...
    set_hostname("getlainux");
    create_user("getlainux");
    setup_network();
...
void show_success_screen(int success, DiskInfo *disk) {
    ...
    if (success) {
        ...
        mvprintw(y/2 - 1, (x-40)/2, "GetLainux successfully installed on");
        mvprintw(y/2, (x-40)/2, "/dev/%s", disk->name);
        mvprintw(y/2 + 2, (x-40)/2, "Default credentials:");
        mvprintw(y/2 + 3, (x-40)/2, "  Username: getlainux");
        mvprintw(y/2 + 4, (x-40)/2, "  Password: getlainux");
        mvprintw(y/2 + 6, (x-40)/2, "Reboot to start using GetLainux");
    } else {
...
void run_turbo_install(void) {
    ...
    mvprintw(y/2 - 2, (x-30)/2, "TURBO INSTALL GETLAINUX");
    ...
    if (disk_idx != -1) {
        ...
        if (strcmp(confirm, "TURBO") == 0) {
            ...
            if (install_result == 0) {
                clear();
                mvprintw(10, 10, "Reboot now to start GetLainux? (y/N): ");
                echo();
                char reboot[2];
                getstr(reboot);
                noecho();
...
int main() {
    ...
    while(running) {
        ...
        switch(res) {
            case 0: {  // Turbo Install GetLainux
                run_turbo_install();
                break;
            }
            case 1: {  // Manual Installation
                if (!check_internet()) {
                    ...
                    mvprintw(11, 10, "GetLainux requires internet to download packages.");
            ...
            case 4: {  // System Deployment (Future feature)
                ...
                mvprintw(12, 10, "Deploy pre-configured GetLainux systems");
                mvprintw(14, 10, "Available in next version");
                mvprintw(16, 10, "Press any key to continue...");
                getch();
                break;
    ...
    endwin();
    printf("\nGetLainux Installer - Installation complete.\n");
    printf("Thank you for using GetLainux!\n");
    return 0;
}
