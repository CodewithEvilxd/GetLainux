...
void emergency_cleanup() {
    ...
    run_command("rm -f /tmp/getlainux-*.tmp 2>/dev/null || true", 0);
...
int download_file(const char *url, const char *output) {
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "GetLainux-Installer/1.0");
...
void download_arch_iso() {
    log_message("Downloading GetLainux ISO...");

    // Check existing file
    if (file_exists("getlainux.iso")) {
        log_message("Existing ISO found, checking integrity...");
        long size = 0;
        struct stat st;
        if (stat("getlainux.iso", &st) == 0) {
            size = st.st_size;
        }
    ...
    snprintf(cmd, sizeof(cmd), "wget -c --timeout=30 --tries=3 '%s' -O getlainux.iso 2>&1 | grep --line-buffered -E '([0-9]+)%%|speed'", ARCH_ISO_URL);
    int result = run_command(cmd, 1);

    if (result != 0) {
        log_message("wget failed, trying curl...");
        snprintf(cmd, sizeof(cmd), "curl -L -C - --connect-timeout 30 --retry 3 '%s' -o getlainux.iso 2>&1 | grep --line-buffered -E '([0-9]+[.][0-9]*%%)|speed'", ARCH_ISO_URL);
        run_command(cmd, 1);
    }
    ...
    if (file_exists("getlainux.iso")) {
        ...
        log_message("Failed to download GetLainux ISO");
    }
}
...
void select_iso_file(char *iso_path, size_t size) {
    ...
    attron(COLOR_PAIR(2));
    mvprintw(5, 15, "1. Download latest GetLainux ISO");
    attroff(COLOR_PAIR(2));
    ...
    switch (choice[0]) {
        case '1': {
            clear();
            mvprintw(5, 10, "Downloading latest GetLainux ISO...");
            ...
            if (file_exists("getlainux.iso")) {
                strncpy(iso_path, "getlainux.iso", size);
                ...
                if (stat("getlainux.iso", &st) == 0) {
                    ...
                    mvprintw(6, 10, "File: getlainux.iso");
...
void select_configuration() {
    ...
    while (1) {
        ...
        if (ch == KEY_UP) {
            ...
            // Save configuration
            FILE *config_file = fopen("getlainux-config.txt", "w");
            if (config_file) {
                fprintf(config_file, "# GetLainux Installation Configuration\n");
                ...
                clear();
                mvprintw(5, 10, "Configuration saved to getlainux-config.txt");
...
void perform_installation(const char *disk) {
    ...
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "mkfs.fat -F32 -n GETLAINUX_EFI %s", efi_part);
    ...
    snprintf(cmd, sizeof(cmd), "mkfs.ext4 -F -L getlainux_root %s", root_part);
    run_command(cmd, 1);
    ...
    // Set hostname
    run_command("echo 'getlainux' > /mnt/etc/hostname", 0);
    run_command("echo '127.0.1.1 getlainux.localdomain getlainux' >> /mnt/etc/hosts", 0);

    // Install GetLainux core with fallback
    display_status("Installing GetLainux core...");
    ...
    if (download_result == ERR_SUCCESS) {
        ...
        log_message("Failed to download GetLainux core. Installation will continue without it.");
    }
    ...
    if (file_exists("/mnt/usr/bin/grub-install")) {
        snprintf(cmd, sizeof(cmd), "arch-chroot /mnt grub-install --target=x86_64-efi --efi-directory=/boot --bootloader-id=getlainux --recheck");
    } else {
        snprintf(cmd, sizeof(cmd), "grub-install --target=x86_64-efi --efi-directory=/mnt/boot --bootloader-id=getlainux --recheck");
    }

    if (run_command(cmd, 1) != 0) {
        log_message("GRUB installation failed, trying alternative...");
        // Try without target specification
        snprintf(cmd, sizeof(cmd), "arch-chroot /mnt grub-install --efi-directory=/boot --bootloader-id=getlainux");
        run_command(cmd, 1);
    }
    ...
    run_command("echo 'root:getlainux' | arch-chroot /mnt chpasswd", 0);

    // Create a regular user
    run_command("arch-chroot /mnt useradd -m -G wheel -s /bin/bash getlainux", 0);
    run_command("echo 'getlainux:getlainux' | arch-chroot /mnt chpasswd", 0);
...
void show_summary(const char *disk) {
    ...
    mvprintw(6, center_x + 5, "GetLainux has been successfully installed!");
    ...
    mvprintw(13, center_x + 10, "Password: getlainux");

    mvprintw(14, center_x + 10, "Username: getlainux");
    mvprintw(15, center_x + 10, "Password: getlainux");
    ...
    mvprintw(23, center_x + 10, "4. Run 'getlainux-setup' for post-installation");
                 ...
                 case 6: // Network Check
                    ...
                    case 7:
                       ...
                       print_settings();
                        ...
                        break;
                    case 8: // Exit
                        if (confirm_action("Exit GetLainux installer?", "EXIT")) {
                            cleanup_ncurses();
                            curl_global_cleanup();
                            return 0;
                        }
