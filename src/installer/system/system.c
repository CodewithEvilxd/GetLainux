...
int install_grub(const char *disk_name) {
    ...
    if (access("/sys/firmware/efi", F_OK) == 0) {
        ...
        snprintf(cmd, sizeof(cmd), 
                 ...
                 "--efi-directory=/boot/efi --bootloader-id=GETLAINUX --recheck %s",
                 disk_path);
    } else {
...
int auto_configure_system(void) {
    ...
    system("echo 'getlainux' > /mnt/etc/hostname 2>/dev/null");
    
    // Configure hosts file
    system("echo '127.0.0.1 localhost' > /mnt/etc/hosts 2>/dev/null");
    system("echo '::1 localhost' >> /mnt/etc/hosts 2>/dev/null");
    system("echo '127.0.1.1 getlainux.localdomain getlainux' >> /mnt/etc/hosts 2>/dev/null");
    ...
    system("arch-chroot /mnt useradd -m -G wheel,audio,video,storage -s /bin/bash getlainux 2>/dev/null");
...
int turbo_install(const char *disk_name) {
    ...
    printf("\n");
    printf("Reboot to start using GetLainux\n");
