...
void create_virtual_disk() {
    ...
    run_command("qemu-img create -f qcow2 -o compression_type=zlib getlainux-vm.qcow2 20G", 1);

    log_message("Virtual disk created: getlainux-vm.qcow2");
}
...
void install_on_virtual_machine() {
    ...
    mvprintw(4, 10, "This will create a QEMU virtual machine for GetLainux.");
    ...
    attron(COLOR_PAIR(2));
    mvprintw(8, 30, "getlainux-vm.qcow2        - 20GB virtual disk");
    mvprintw(9, 30, "install-getlainux-vm.sh   - VM startup script");
    mvprintw(10, 30, "VM-README.txt         - Instructions");
    attroff(COLOR_PAIR(2));

    mvprintw(12, 25, "To start the virtual machine:");
    attron(A_BOLD);
    mvprintw(13, 30, "sudo ./install-getlainux-vm.sh");
    attroff(A_BOLD);
...
void setup_qemu_vm(const char *iso_path) {
    ...
    FILE *script = fopen("install-getlainux-vm.sh", "w");
    if (script) {
        fprintf(script, "#!/bin/bash\n");
        fprintf(script, "# GetLainux VM Installation Script\n");
        fprintf(script, "# Generated on: %s", ctime(&(time_t){time(NULL)}));
        fprintf(script, "\n");
        fprintf(script, "echo 'Starting GetLainux VM installation...'\n");
        ...
        fprintf(script, "  -drive file=getlainux-vm.qcow2,format=qcow2 \\\n");
        ...
        fprintf(script, "  -name \"GetLainux-VM\"\n");

        fclose(script);
        run_command("chmod +x install-getlainux-vm.sh", 0);
    }

    // Create README file
    script = fopen("VM-README.txt", "w");
    if (script) {
        fprintf(script, "GetLainux Virtual Machine Installation\n");
        fprintf(script, "====================================\n\n");
        fprintf(script, "Files created:\n");
        fprintf(script, "1. getlainux-vm.qcow2    - Virtual disk (20GB)\n");
        fprintf(script, "2. install-getlainux-vm.sh - Installation script\n\n");
        fprintf(script, "To start the virtual machine:\n");
        fprintf(script, "  sudo ./install-getlainux-vm.sh\n\n");
