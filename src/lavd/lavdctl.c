...
int started_lavd_system()
{
    ...
    system("sudo lavdctl diagnose full --output /tmp/getlainux-diag-$(date +%s).json");

    int layer;
    for(layer = 0; layer < 4; layer++) {
        DRV_INFO("checking layer %d\n", layer);
        system("/usr/local/bin/getlainux-lavd-integration.sh layer%layer");
    }

    DRV_INFO("lets all love lain\n");
    DRV_OK("getlainux lavd integration\n");
